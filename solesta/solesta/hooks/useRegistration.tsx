...
export function RegistrationProvider({ children }: { children: ReactNode }) {
  ...
  const startRegistration = useCallback(
    async (institution: "krmu" | "external") => {
      setIsLoading(true)
      setError(null)
  ...
  const submitRollNumber = useCallback(
    async (rollNumber: string) => {
      setIsLoading(true)
      setError(null)
      ...
      if (result.success && result.data) {
        ...
        if (data.state === "ROLL_NUMBER_CONFIRM") {
          ...
          if (data.state === "FRESHER_SELECTION") {
            ...
            data.state === "ENTER_NAME" ||
            data.state === "MANUAL_DETAILS"
          ) {
            advanceStep("details")
          }
        } else {
  ...
  const submitDetails = useCallback(
    async (name: string, course: string, year: string, college?: string) => {
      setIsLoading(true)
      setError(null)

      const result = await api.submitDetails(
        externalAppId,
        name,
        course,
        year,
  ...
  const submitFresher = useCallback(
    async (fresher: boolean) => {
      setIsLoading(true)
      setError(null)
