...
import { createRegistration, getRegistrationByExternalAppId, getRegistrationByReferenceId, checkDuplicateRegistration } from '@/lib/server/registration';
...
async function handleRollNumber(body: any) {
  ...
  if (!externalAppId || !rollNumber) {
    return NextResponse.json(
      { success: false, error: 'MISSING_PARAM', message: 'externalAppId and rollNumber are required.' },
      { status: 400 }
    );
  }
  ...
  if (!/^\d{10}$/.test(rollNumber)) {
    return NextResponse.json(
      { success: false, error: 'INVALID_FORMAT', message: 'Roll number must be 10 digits.' },
      { status: 400 }
    );
  }
  ...
  if (!user) {
    return NextResponse.json(
      { success: false, error: 'USER_NOT_FOUND', message: 'Please start with /start.' },
      { status: 400 }
    );
  }
  ...
  if (!student) {
    return NextResponse.json(
      { success: false, error: 'STUDENT_NOT_FOUND', message: 'Roll number not found in database.' },
      { status: 400 }
    );
  }
  ...
  await updateStateData({ externalAppId }, {
    rollNumber,
  ...
  const alreadyVerified = await isEmailVerifiedRecently(externalAppId, deviceToken, 48);
  if (alreadyVerified.valid) {
    return handleOtpVerifyAfterSkip(externalAppId, user, { ...user.stateData, institution: 'krmu', email: student.email || `${rollNumber}@krmu.edu.in`, name: student.name, course: student.courseAndSemester, year: student.year }, deviceToken);
  }
...
async function handleEmailInput(body: any) {
  ...
  if (!externalAppId || !email) {
    return NextResponse.json(
      { success: false, error: 'MISSING_PARAM', message: 'externalAppId and email are required.' },
      { status: 400 }
    );
  }
  ...
  if (!emailRegex.test(email)) {
    return NextResponse.json(
      { success: false, error: 'INVALID_FORMAT', message: 'Invalid email format.' },
      { status: 400 }
    );
  }
  ...
  const existingRegistration = await getRegistrationByExternalAppId(externalAppId);
...
async function handleOtpRequest(body: any) {
  ...
  if (!email) {
    return NextResponse.json(
      { success: false, error: 'NO_EMAIL', message: 'No email found. Please complete previous step.' },
      { status: 400 }
    );
  }
  ...
  return NextResponse.json({ success: true, data: { otpSent: true }, message: result.message });
}
...
async function handleOtpVerify(body: any) {
  ...
  if (!externalAppId || !otp) {
    return NextResponse.json(
      { success: false, error: 'MISSING_PARAM', message: 'externalAppId and otp are required.' },
      { status: 400 }
    );
  }
...
async function handleDetails(body: any) {
  ...
  const verificationCheck = await requireEmailVerification(externalAppId, deviceToken, 48);
  if (!verificationCheck.valid) {
    return NextResponse.json(
      { success: false, error: 'EMAIL_NOT_VERIFIED', message: verificationCheck.message },
      { status: 401 }
    );
  }
  ...
  if (!stateData.email) {
    return NextResponse.json(
      { success: false, error: 'NO_EMAIL', message: 'Please provide email first.' },
      { status: 400 }
    );
  }
  ...
  if (!name || name.trim().length < 2) {
    return NextResponse.json(
      { success: false, error: 'INVALID_NAME', message: 'Name must be at least 2 characters.' },
      { status: 400 }
    );
  }
  if (!course || course.trim().length < 2) {
    return NextResponse.json(
      { success: false, error: 'INVALID_COURSE', message: 'Course must be at least 2 characters.' },
      { status: 400 }
    );
  }
  ...
  const duplicateCheck = await checkDuplicateRegistration(undefined, stateData.email);
  if (duplicateCheck.exists) {
    return NextResponse.json(
      { 
        success: false, 
...
async function handleFresher(body: any) {
  ...
  if (typeof isFresher !== 'boolean') {
    return NextResponse.json(
      { success: false, error: 'INVALID_PARAM', message: 'isFresher must be true or false.' },
      { status: 400 }
    );
  }
  ...
  const stateData = user.stateData as any;
  if (!stateData.name || !stateData.course || !stateData.year || !stateData.email) {
    return NextResponse.json(
      { success: false, error: 'INCOMPLETE_DATA', message: 'Registration data incomplete. Please start again.' },
      { status: 400 }
    );
  }

  // Check for duplicate registration by roll number (KRMU) or email
  const duplicateCheck = await checkDuplicateRegistration(stateData.rollNumber, stateData.email);
  if (duplicateCheck.exists) {
    return NextResponse.json(
      { 
        success: false, 
...
async function handleSearchByReference(referenceId: string): Promise<NextResponse> {
  try {
...
async function handleConfirmPayment(body: any) {
  ...
  if (!referenceId) {
    return NextResponse.json(
      { success: false, error: 'MISSING_PARAM', message: 'referenceId is required.' },
      { status: 400 }
    );
  }
  ...
  await updateUserState({ externalAppId: registration.externalAppId }, UserState.PAYMENT_CONFIRMED);
  ...
  if (isKrmu) {
    ...
    if (isFirstYear) {
      ...
      return NextResponse.json({
        ...
        message: 'Email verified within 48 hours. Would you like to participate in Mr. & Mrs. Fresher?',
      });
    }
