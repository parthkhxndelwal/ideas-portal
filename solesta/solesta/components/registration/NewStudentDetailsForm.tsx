...
export function NewStudentDetailsForm() {
  const { submitNewStudentDetails, goBack, isLoading, error } =
    useRegistration()
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (name.length >= 2 && course.length >= 2 && year) {
      submitNewStudentDetails(name, course, year)
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        This roll number is not in our database. Please provide your details and
        we'll add you.
      </p>
