...
import { generateOtp, hashCode, verifyCode } from '../utils/crypto';
...
export async function createAndSendOtp(
  ...
  logger.info('OTP created and sent', { userId, email });
...
export async function verifyOtp(userId: string, code: string): Promise<VerifyOtpResult> {
  ...
  if (otpRecord.retryCount >= config.otpMaxRetries) {
    return { success: false, message: 'Maximum retry limit exceeded. Please request a new OTP.' };
  }
...
export async function canRequestOtp(userId: string): Promise<{ allowed: boolean; message: string }> {
  const otpRecord = await prisma.otp.findFirst({
    where: { userId, isUsed: false },
    orderBy: { createdAt: 'desc' },
  });
  ...
  if (otpRecord.retryCount >= config.otpMaxRetries) {
    return { allowed: false, message: 'Maximum retry limit exceeded. Please request a new OTP.' };
  }
